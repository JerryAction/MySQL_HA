...
sub get_alive_servers_not_slave($) {
  my $self = shift;
  my @alive_servers = $self->get_alive_servers();
  my @alive_masters;
  foreach my $server (@alive_servers) {
    if ($server->{not_slave} eq '1') {
      push @alive_masters, $server;
    }
  }
...
"There are $not_slave_servers non-slave servers! MHA manages at most one non-slave server. Check configurations."
  ...
  foreach my $key ( keys(%master_hash) ) {
    ...
    unless ($master) {
      $log->warning(
        sprintf(
"Master %s:%d from which slave %s replicates is not defined in the configuration file. This master will be ignored for failover.",
          $slave->{Master_IP}, $slave->{Master_Port},
          $slave->get_hostinfo()
        )
      );
      # Remove this slave from master_hash to ignore it
      delete $master_hash{$key};
      next;
    }
  }

  # Check if we still have valid masters after filtering out undefined ones
  if (keys(%master_hash) < 1) {
    $log->error("No valid masters found after filtering out undefined masters. Check configurations.");
    croak;
  }
...
sub validate_master_ip_port {
  ...
  foreach my $slave (@alive_servers) {
    # Check if either id is undefined before comparison
    if ( !defined($slave->{id}) || !defined($real_master->{id}) || $slave->{id} eq $real_master->{id} ) {
      next;
    }
    ...
    # Modified to allow master to be a slave of another host
    if (
      !(
           ( defined($slave->{Master_IP}) && defined($real_master->{ip}) && $slave->{Master_IP} eq $real_master->{ip} )
        && ( defined($slave->{Master_Port}) && defined($real_master->{port}) && $slave->{Master_Port} == $real_master->{port} )
      )
    )
    {
      if ( $slave->{multi_tier_slave} ) {
        ...
        # Allow slave to replicate from a different master (master can be a slave of another host)
        my $msg = sprintf(
          "Slave %s replicates from %s:%d, which is allowed in modified MHA architecture (real master is %s)",
          $slave->get_hostinfo(), $slave->{Master_Host},
          $slave->{Master_Port},  $real_master->get_hostinfo()
        );
        $log->info($msg);
        # Do not croak here, allow the setup
      }
    }
  }
...
sub get_multi_master_print_info {
  ...
  # Handle case where master_hash is empty
  if (keys(%master_hash) < 1) {
    $str = "No valid masters found.\n";
    return $str;
  }
...
sub get_primary_master {
  ...
  # Check for circular replication setup
  my $is_circular = 0;
  if (scalar(@masters) == 2) {
    my $master1 = $masters[0];
    my $master2 = $masters[1];
    
    # Check if it's a circular replication (each master replicates from the other)
    if (($master1->{Master_IP} eq $master2->{ip} && $master1->{Master_Port} == $master2->{port}) &&
        ($master2->{Master_IP} eq $master1->{ip} && $master2->{Master_Port} == $master1->{port})) {
      $is_circular = 1;
      # In circular replication, we can treat either one as the primary master
      $real_master = $master1;
      $num_real_masters = 1;
    }
  }
...
"Multi-master configuration is detected, but all of them are read-only! Check configurations for details. Master configurations are as below: \n%s",
  ...
  elsif ( $num_real_masters >= 2 && !$is_circular ) {
    $log->error(
      sprintf(
"Multi-master configuration is detected, but two or more masters are either writable (read-only is not set) or dead! Check configurations for details. Master configurations are as below: \n%s",
    ...
    if ($is_circular) {
      $log->info(
        sprintf(
"Circular multi-master configuration is detected. Current primary master is %s",
...
sub get_current_alive_master($) {
  ...
  my $master = $self->get_orig_master();
  unless ($master) {
      # 新增：如果之前没指定 orig_master，就把第一个 candidate_master 拿来做主库
      my @candidates = $self->get_candidate_masters();
      if (@candidates) {
          $master = $candidates[0];
          $self->set_orig_master($master);
          $log->warning(
            "orig_master was not set; automatically picked $master->{hostname}:$master->{port}"
          );
      } else {
          $log->error(
            "MySQL master is not correctly configured. Check master/slave settings");
