...
sub wait_until_master_is_dead {
  ...
  $exit_code = eval {
    ...
    my $is_circular = 0;
    if ($real_master && $dead_master && 
        $real_master->{Master_IP} && $dead_master->{ip} &&
        $real_master->{Master_IP} eq $dead_master->{ip} && 
        $real_master->{Master_Port} == $dead_master->{port} &&
        $dead_master->{Master_IP} && $real_master->{ip} &&
        $dead_master->{Master_IP} eq $real_master->{ip} && 
        $dead_master->{Master_Port} == $real_master->{port}) {
      $is_circular = 1;
    }
    
    # Handle case where real_master is not set (e.g., in circular replication)
    if (!$real_master && $is_circular) {
      # In circular replication, we can use the alive master as real_master
      my @alive_masters = $_server_manager->get_alive_servers_not_slave();
      if (scalar(@alive_masters) > 0) {
        $real_master = $alive_masters[0];
      }
    }
    
    # Check if either id is undefined and not in circular replication
    if ( (!$dead_master->{id} || !$real_master->{id}) && !$is_circular ) {
      $log->error(
        sprintf(
"Monitor detected %s failed, but actual master server is not properly defined. Check replication configurations again.",
          $dead_master->get_hostinfo()
        )
      );
      return 1;
    ...
    # Additional check for undefined values before comparison
    if ( (!defined($dead_master->{id}) || !defined($real_master->{id})) && !$is_circular ) {
      $log->error(
        sprintf(
"Monitor detected %s failed, but actual master server is not properly defined. Check replication configurations again.",
          $dead_master->get_hostinfo()
    ...
    if ( $dead_master->{id} ne $real_master->{id} && !$is_circular ) {
      $log->error(
        sprintf(
"Monitor detected %s failed, but actual master server is %s. Check replication configurations again.",
    ...
    # In circular replication, we allow the "real master" to be the one that replicates from the dead master
    if ($is_circular) {
      # Check if $real_master is defined before using it
      if (!$real_master) {
        $log->error(
          "Circular replication detected, but real master is not properly defined. " .
          "Check replication configurations again."
        );
        return 1;
      }
      $log->info(
        sprintf(
"Circular replication detected. %s failed, but %s is the original master.",
          $dead_master->get_hostinfo(),
          $real_master->get_hostinfo()
        )
      );
